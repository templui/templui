...
func initConfig(ref string, force bool) {
	...
	if configExists {
		...
		if !force {
			...
			_, err := loadConfig()
			if err != nil {
				fmt.Println("Config file exists but has issues. Use 'templui --force init' to repair missing fields and reinstall utils.")
				return
			}
			fmt.Println("Config file already exists and is complete. Use 'templui --force init' to reinstall utils if needed.")

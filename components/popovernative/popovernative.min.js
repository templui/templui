(function () {
  ...
  function closeAll(exceptId = null) {
    document.querySelectorAll("[data-tui-popovernative-content]").forEach((content) => {
  ...
  function handleHoverEnter(trigger, id) {
    ...
    const delay =
      parseInt(content.getAttribute("data-tui-popovernative-hover-delay"), 10) ||
      100;
    const timeouts = hoverTimeouts.get(id) || {};
    clearTimeout(timeouts.leave);
    clearTimeout(timeouts.enter);
  ...
  function handleHoverLeave(id, content) {
    if (!content) return;

    const delay =
      parseInt(content.getAttribute("data-tui-popovernative-hover-out-delay"), 10) ||
      200;
    const timeouts = hoverTimeouts.get(id) || {};
    clearTimeout(timeouts.enter);
  ...
  document.addEventListener("click", (event) => {
    ...
    document.querySelectorAll("[data-tui-popovernative-content]").forEach((content) => {
      if (
        ...
        content.getAttribute("data-tui-popovernative-disable-clickaway") === "true"
      ) {
        return;
      }
      ...
      const clickedTrigger = getTriggers(id).some((item) => item.contains(event.target));
  ...
  document.addEventListener("mouseover", (event) => {
    ...
    if (content && !content.contains(event.relatedTarget) && content.matches(":popover-open")) {
      const id = content.getAttribute("data-tui-popovernative-id");
      const timeouts = hoverTimeouts.get(id) || {};
      clearTimeout(timeouts.leave);
      hoverTimeouts.set(id, timeouts);
  ...
  document.addEventListener("mouseout", (event) => {
    ...
    if (
      ...
      const id = trigger.getAttribute("data-tui-popovernative-trigger");
      const content = id ? getContent(id) : null;
      if (id && !content?.contains(event.relatedTarget)) {
        handleHoverLeave(id, content);
      }
    }
    ...
    if (content && !content.contains(event.relatedTarget) && content.matches(":popover-open")) {
      const id = content.getAttribute("data-tui-popovernative-id");
      const movingToTrigger = getTriggers(id).some((item) =>

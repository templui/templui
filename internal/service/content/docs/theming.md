...
Dark mode works by overriding the same tokens inside a `.dark` selector. See the [dark mode docs](/docs/dark-mode) for adding the theme script and toggling the `.dark` class.
...
```templ /bg-warning/ /text-warning-foreground/
...
```css showLineNumbers title="assets/css/globals.css"
@import "tailwindcss";

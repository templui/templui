...
We read a lot about type: scale ratios, tracking, kerning, optical sizing, measure, leading, the space above and below every element. shadcn tried exposing all of it, and it was too much. Nobody wants to set a dozen variables to make markdown look right.

So they sat down and condensed everything into three controls: size, leading, and flow. Everything else, heading sizes, list indents, the gap under a heading, the space around a rule, derives from them. Three controls. They called it rhythm.
...
`typeset` turns the styles on. `typeset-docs` is the preset you created.
...
Do this in your renderer's table component or a small goldmark rendering hook. It works for any wide block, not just tables.
...
<div class="typeset">
	<p>Styled prose.</p>
	@card.Card(card.Props{Class: "not-typeset"}) {
		Untouched component.
	}
</div>
```
